...
from sqlalchemy.orm import (DeclarativeBase,
                            Mapped,
                            mapped_column, relationship, Session, selectinload)
...
def setup_database():
    ...
    with Session(engine) as session:
        if session.query(Author).count()==0:
            ...
            ab1 = AuthorBio(bio_text='Британская писательница...', author=a1)
            a2 = Author(name='Агата Кристи')
            ab2 = AuthorBio(bio_text='Мастер детективного жанра...', author=a2)
...
#         book2 = Book(
# # #             name='Фантастические твари и места их обитания', author=author1)
# # #         # add-для одной записи, add_all-список для добавления
...
#  get_all_genres, create_database
def get_all_authors():
    with Session(engine) as session:

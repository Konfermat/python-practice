...
def init_board():
    ...
    for i in range(8):
        board[1][i] = create_piece('♟', 'black')
        board[6][i] = create_piece('♙', 'white')
        # board[y][x]

    board[0][0] = create_piece('♜', 'black')
    board[0][1] = create_piece('♞', 'black')
    board[0][2] = create_piece('♝', 'black')
    board[0][3] = create_piece('♛', 'black')
    board[0][4] = create_piece('♚', 'black')
    board[0][5] = create_piece('♝', 'black')
    board[0][6] = create_piece('♞', 'black')
    board[0][7] = create_piece('♜', 'black')

    board[7][0] = create_piece('♖', 'white')
    board[7][1] = create_piece('♘', 'white')
    board[7][2] = create_piece('♗', 'white')
    board[7][3] = create_piece('♕', 'white')
    board[7][4] = create_piece('♔', 'white')
    board[7][5] = create_piece('♗', 'white')
    board[7][6] = create_piece('♘', 'white')
    board[7][7] = create_piece('♖', 'white')
    ...
    for i in range(8):
        ...
        for j in range(8):
            ...
            if piece:
                ...
                if (i + j) % 2 == 0:
                    row += f"█{symbol}█"  # Символ для черной клетки
                else:
                    row += f"░{symbol}░"  # Символ для белой клетки
            else:
                if (i + j) % 2 == 0:
                    row += f"███"  # Символ для черной клетки
                else:
                    row += f"░░░"
        print(row)
    print(' a  b  c  d  e  f  g  h ')

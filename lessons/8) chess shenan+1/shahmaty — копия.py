...
def user_turn(state):
    while True:
        ...
        try:
            ...
            if not(0 <= fx < 8 and 0 <= fy < 8 or 0 <= tx < 8 or 0 <= ty < 8 ):
                print('неверный ход')
                continue
...
def bot_turn(state):
    ...
    if valid_moves:
        ...
        print(f'Бот сходил: {chr(97+from_pos[0])}{8-from_pos[1]}->{chr(97+to_pos[0])}{8-to_pos[1]}')
    else:
        print('У бота больше нет доступных ходов.')
    return state

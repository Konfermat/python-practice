class SmartHistoryDict:
    ...
    def undo(self, step=1):
        '''откатывает га указанное количество шагов назад'''
    ...
    def search_in_history(self, key):
        '''ищет все изменения определенного ключа в истории'''
        changes = []
        for state in self.history:
            if key in state['data']:
                changes.append({
                    'timestamp': state['timestamp'],#номер шага
                    'value': state['data'][key],#значениме на этом шаге
                    'description': state['description']
                })
        return changes

def test_smart_dict():
    shd = SmartHistoryDict({'name': 'Alex', 'age': 99})

import pytest
from CellularLifeSimulator import Nutrient, HerbivoreCell, CarnivoreCell, Ecosystem
import uuid

# фикстуры (настройка тестовых данных)
...
class TestCellBaseLogic:
    def test_initial_energy(self, basic_herbivore):
        assert basic_herbivore.level_energy == 100
        assert basic_herbivore.is_alive()

    def test_move_consume_energy(self, basic_herbivore):
        # движение уменьшает энергию и меняет координаты
        initial_energy = basic_herbivore.level_energy
        initial_x = basic_herbivore.x
        cost = basic_herbivore.energy_cost
        basic_herbivore.move(2, 1)
        assert basic_herbivore.x == initial_x + 2
        assert basic_herbivore.y == 10 - 1
        assert basic_herbivore.level_energy == initial_energy - cost

    def test_dead_by_energy_zero(self, basic_herbivore):
        # проверяем что клетка умирает когда энергия опускается до нуля
        basic_herbivore.level_energy = 5
        basic_herbivore.move(1, 0)
        assert basic_herbivore.level_energy == 0
        assert not basic_herbivore.is_alive()

    def test_dead_by_cell_cannot_move(self, basic_herbivore):
        # мертвая клетка не может двигаться
        basic_herbivore.level_energy = 0
...
class TestHerbivoreCell:
    def test_interact_eats(self, basic_herbivore, basic_ecosystem):
        # проверяем что потребляет ресурс на той же позиции
        basic_herbivore.x = 5
        basic_herbivore.y = 5
        ...
        assert basic_herbivore.level_energy == initial_energy + metabolism
    ...
    def test_interact_no_nutrient(self, basic_herbivore, basic_ecosystem):
        ...
        # 10, 10 нет ресурсов
        basic_herbivore.interact(basic_ecosystem)
        assert basic_herbivore.level_energy == initial_energy

# тесты для хищника
class TestCarnivoreCell:
    def test_interact_attack_herbivore(self, basic_carnivore, basic_herbivore, basic_ecosystem):
        # проверяем успешную атаку хищника
        # размещаем хищника и жертву в одной точке
        basic_carnivore.x = 5
        basic_carnivore.y = 5
        basic_herbivore.x = 5
        basic_herbivore.y = 5
        ...
        c_initial_energy = basic_carnivore.level_energy
        h_initial_energy = basic_herbivore.level_energy #150

        attack_power = basic_carnivore.attack_power #50

        # атака
        basic_carnivore.interact(basic_ecosystem)
        assert basic_carnivore.level_energy == c_initial_energy + attack_power
        assert basic_herbivore.level_energy == h_initial_energy - attack_power
    ...
    def test_interact_no_target(self, basic_carnivore, basic_ecosystem):
        ...
        basic_carnivore.interact(basic_ecosystem)
        assert basic_carnivore.level_energy == c_initial_energy
    ...
    def test_interact_kills_target(self, basic_carnivore, basic_herbivore, basic_ecosystem):
        ...
        basic_herbivore.level_energy = 30
        basic_carnivore.x = 5
        basic_carnivore.y = 5
        basic_herbivore.x = 5
        basic_herbivore.y = 5
        ...
        basic_herbivore.interact(basic_ecosystem)
        assert basic_herbivore.level_energy == 0
        assert basic_carnivore.level_energy == c_initial_energy + 30
...
class TestEcosystem:
    ...
    def test_run_cycle(self, basic_ecosystem):
        ...
        herb = HerbivoreCell(0, 0) # 1, 0
        carn = CarnivoreCell(1, 0)

        initial_h_energy = herb.level_energy # 150
        initial_c_energy = carn.level_energy # 200
        cost = herb.energy_cost # 5
        attack = carn.attack_power
        ...
        h_energy = herb_collision.level_energy # 150
        c_energy = carn_collision.level_energy # 200
        ...
        # взаимодействие
        assert herb_collision.level_energy == h_energy - cost - attack
        assert carn_collision.level_energy == c_energy - cost + attack
        assert num_cells == 2

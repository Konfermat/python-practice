...
class HRConsole:
        ...
        while True:
            ...
            if choice == '6':
                ...
                if DBManagerSingleton._connection:
                    DBManagerSingleton.connection.close()
                break
    ...
    def show_all_employees(self):
        print('1. Отчет: Полный список сотрудников, отделов и mail')
        query = '''
    ...
    def show_employee_project(self):
        ...
        try:
            with self.db_manager.connection() as conn:
                ...
                cursor = conn.execute(quary, params)
                result = cursor.fetchall()
                if not result:
...
#(9, 't'), (9, 'A'), (5, 'H')
    ...
    def assign_employee_to_dep(self):
        try:
            ...
            print('Должен быть целыми числами')
            return
        # select 1 from Emplouees where employee_id = ?
        update_quary = '''
        ...
        try:
            with self.db_manager.connection() as conn:
                conn.execute(update_quary, (dep_id, employee_id))
        except Exception as e:
            print(f'Не удалось выполнить запрос: {e}')
...
# app.show_employees_without_details()
# app.show_employee_project()
# app.assign_employee_to_dep()
# app.show_department_summary()
